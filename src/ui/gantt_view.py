...
class GanttTaskRow(QWidget):
    ...
    def on_task_drag_moved(self, task_data, days_moved):
        ...
        if parent and isinstance(parent, GanttChart):
            # 直接使用scheduler_manager更新任务
            if parent.scheduler_manager.update_task(updated_task):
                # 更新task_data引用以保持同步
...
class GanttChart(QTableWidget):
    ...
    def __init__(self, start_date, end_date, tasks, scheduler_manager, parent=None):
        ...
        # 设置表格属性
        self.setColumnCount(1)  # 初始只设置一列，后面会动态添加日期列
        ...
        header.setMinimumSectionSize(self.day_width)
    ...
    def setup_table(self):
        ...
        # 设置列数（任务名称列 + 日期列）
        self.setColumnCount(total_days + 1)
        
        # 设置任务名称列
        title_item = QTableWidgetItem("任务")
        title_item.setTextAlignment(Qt.AlignCenter)
        self.setHorizontalHeaderItem(0, QTableWidgetItem(title_item))
        ...
        for i in range(total_days):
            ...
            if self.day_width < 35:
                # 简化格式
                date_text = current_date.strftime("%d")
                if self.day_width >= 25:
                    date_text += "\n" + current_date.strftime("%a")[0]
            else:
                # 完整格式
                date_text = current_date.strftime("%m-%d\n%a")
            
            header_item.setText(date_text)
            
            # 设置周末列的背景色
            if current_date.weekday() >= 5:
                header_item.setBackground(QColor(248, 248, 248))
            
            self.setHorizontalHeaderItem(i + 1, header_item)
            current_date += timedelta(days=1)
        
        # 设置行数
        self.setRowCount(len(self.tasks))
        ...
        for i, task in enumerate(self.tasks):
            ...
            start_col = (task_start - self.start_date).days + 1
            duration = (task_end - task_start).days + 1
            
            # 创建任务条
            task_bar = GanttTaskBar(task, self.start_date, self.day_width)
            task_bar.clicked.connect(lambda t=task: self.edit_task(t))
            task_bar.dragMoved.connect(lambda t, d: self.on_task_drag_moved(t, d))
            task_bar.resized.connect(lambda t, d, s: self.on_task_resized(t, d, s))
            
            # 将任务条添加到单元格
            task_layout.addWidget(task_bar)
            self.setCellWidget(i, start_col, task_cell)
            
            # 合并任务条所跨的单元格
            self.setSpan(i, start_col, 1, duration)
            
            # 存储任务行引用
            self.task_rows[task.get("id")] = i
    ...
    def paintEvent(self, event):
        ...
        if self.today_marker:
            ...
            if self.start_date <= today <= self.end_date:
                ...
                # 计算今日线的位置
                days_from_start = (today - self.start_date).days
                x = self.columnViewportPosition(days_from_start + 1)
    ...
    def updateDayWidth(self, new_width):
        ...
        # 更新所有任务条
        self.update_task_bars()
        
    def update_task_bars(self):
        """更新所有任务条"""
        for i in range(self.rowCount()):
            for j in range(1, self.columnCount()):
                widget = self.cellWidget(i, j)
                if widget:
                    task_bar = widget.findChild(GanttTaskBar)
                    if task_bar:
                        task_bar.day_width = self.day_width
                        task_bar.update()
